...
public class CalculatorService {
	...
	public Map<String, String> getCalcTypes(){
		...
		Map<String, String> calcTypes = new LinkedHashMap<String, String>();
	...
	public String convertPysicalCalcType(String calcType) {
		...
		final int ADD_INDEX = 0;
		final int SUB_INDEX = 1;
		final int MUL_INDEX = 2;
		final int DIV_INDEX = 3;
		
		//pysicalCalcTypeの取得処理
		String pysicalCalcType = "";
		String[] calcTypeNameArray = getCalcTypeNameArray();

		for(int i = 0;i < calcTypeNameArray.length;i++) {
			if(calcType.equals("sub")) {
				pysicalCalcType = calcTypeNameArray[SUB_INDEX];
			}else if (calcType.equals("mul")) {
				pysicalCalcType = calcTypeNameArray[MUL_INDEX];
			}else if (calcType.equals("div")) {
				pysicalCalcType = calcTypeNameArray[DIV_INDEX];
			}else {
				pysicalCalcType = calcTypeNameArray[ADD_INDEX];
			}
		}
